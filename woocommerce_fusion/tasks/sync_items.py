...
class ERPNextItemToSync:
    """Track an ERPNext Item and the relevant WooCommerce Server to sync to."""
...
class SynchroniseItem(SynchroniseWooCommerce):
    """
    Manage synchronisation of a WooCommerce Product with an ERPNext Item.
    """
        ...
        if item and self.woocommerce_product:
            ...
            if wc_server.item_field_map:
                ...
                for map in wc_server.item_field_map:
                    ...
                    # Expect woocommerce_field_name to be valid JSONPath
    ...
    def set_product_fields(
        ...
        """
        Push ERPNext values into WooCommerce when field mappings exist on the `WooCommerce Server`.
        Returns True if the WooCommerce product document was changed.
        """
        wc_product_dirty = False
        if item and woocommerce_product:
            ...
            if wc_server.item_field_map:
                ...
                # in-place updates on the whole dict using jsonpath-ng.
                ...
                for map in wc_server.item_field_map:
                    ...
                    # Determine once per mapping: variation and whether the target path is parent-only
                    ...
                    # 0) Variations never carry categories/tags on the WC side → skip early
                    ...
                    # 1) Parse JSONPath and look up matches on the deserialized product structure
                    jsonpath_expr = parse(map.woocommerce_field_name)
                    matches = jsonpath_expr.find(wc_product_with_deserialised_fields)

                    # 2) Handle missing fields:
                    #    - For new WC products (no .name yet), structure may be incomplete → skip silently
                    #    - For categories/tags, absence is legitimate → never hard-fail
                    if not matches:
                    ...
                    # 3) Compare and update if different (take the first match)
...
def get_list_of_wc_products(
    ...
    """
    Fetch a list of WooCommerce Products within a specified date range or linked with an Item, using pagination.
